...
def test_XYplot(db):
    ...
    fig, ax = subplots_create(figsize=(6, 4), style="matplotlib")
    ax = XYplot(df=db.df_combined, x=db.datasets.samples.Condition[0], y=db.datasets.samples.Condition[1], ax=ax)
    fig.show()
...
def test_MAplot(db):
    ...
    fig, ax = subplots_create(figsize=(6, 4), style="matplotlib")
    ax = MAplot(df=db.df_combined, x=db.datasets.samples.Condition[0], y=db.datasets.samples.Condition[1], ax=ax)
    fig.show()
