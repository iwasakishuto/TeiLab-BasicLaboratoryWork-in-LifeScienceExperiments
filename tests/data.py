...
class TestData:
    ...
    def create(self, datasets):
        ...
        for no in range(2):
            ...
            df_combined = pd.concat(
                [
                    df_combined,
                    df_data[[datasets.TARGET_COLNAME]].rename(
                        columns={datasets.TARGET_COLNAME: datasets.samples.Condition[no]}
                    ),
                ],
                axis=1,
            )
    ...
    def generate_normal_distributions(random_state=None, n_samples=4, n_features=1000):
        return np.random.RandomState(random_state).normal(
            loc=np.expand_dims(np.arange(n_samples), axis=1), size=(n_samples, n_features)
        )
