...
def test_pcr_analysis():
    ...
    _,e_REF = calibration_curve_plot(
        qualities=pcr_result.qualities, cts=pcr_result.REF_calibration_Cts, 
        ax=fig.add_subplot(gs[0,3]), target=pcr_result.REF_name
    )
    _,e_GOI = calibration_curve_plot(
        qualities=pcr_result.qualities, cts=pcr_result.GOI_calibration_Cts, 
        ax=fig.add_subplot(gs[1,3]), target=pcr_result.GOI_name
    )
    ax = expression_ratio_plot(
        GOI_Cts=pcr_result.GOI_Cts, REF_Cts=pcr_result.REF_Cts,
        e_GOI=e_GOI, e_REF=e_REF, 
        name_GOI=pcr_result.GOI_name, name_REF=pcr_result.REF_name, 
        labels=pcr_result.samples, ax=fig.add_subplot(gs[:,:3])
    )
    fig.show()
